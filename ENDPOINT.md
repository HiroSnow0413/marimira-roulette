...
| `comment` | string | **no** | 任意の一言メモ。最大 120 文字。クライアント側では `<input maxlength="120">` で制限し、サーバー側でも 120 字で切り詰める（[Code.gs:78-79](Code.gs#L78-L79)）。フィールド省略 or 非文字列は空文字扱い |
...
`ss.getSheetByName('ログ')` を取得し、無ければ `insertSheet('ログ')` した上でヘッダー行を追加（[Code.gs:93-98](Code.gs#L93-L98)）：

```
日付 | 時間 | 項目 | 名前 | 緯度 | 経度 | コメント
```

既存の 6 列ヘッダー（コメント列なし）のシートが存在する場合は G1 に「コメント」を自動追記する後方互換処理あり（[Code.gs:95-97](Code.gs#L95-L97)）。

その後、1 行追加（[Code.gs:102-103](Code.gs#L102-L103)）：
...
| コメント | `comment`（省略時は空文字） | そのまま（サーバー側で 120 字に切り詰め済み） |
...
参考実装: [src/script.js:175-186](src/script.js#L175-L186)
...
列順序は上記の通り固定（A〜G 列、コメント含む 7 列）。行は追記のみ。
